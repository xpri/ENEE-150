...
void main()
{
    ...
    while (strcmp(UserInput, "exit") != 0)
    {
        ...
        if ((GetVectorIndexByLetter(tokensOfUserInput[0]) != -1))
        {
            ...
            if (0 == isSizeAnInteger(tokensOfUserInput[1]))
            {
                printf("Error. Invalid vector length!\n");
            }
            else if (tokensOfUserInput[1] == NULL)
            {
                printf("Error. No vector length specified!\n");
            }
            else if (tokensOfUserInput[2] == NULL)
            {
            ...
            else if (1 == isSizeAnInteger(tokensOfUserInput[1]))
            {
                ...
                vector_new(elements, &SizeOfArray, NumNameOfVector); //This function creates a new vector. It takes the 2D array of doubles, the size of the array and the name of the vector as parameters.
            }
        }
        else if (strcmp(tokensOfUserInput[0], "prn") == 0)
        {
            vector_prn(elements, &SizeOfArray);
        }
        else if (strcmp(tokensOfUserInput[0], "sum") == 0)
        {
            vector_sum(elements, &SizeOfArray);
        }
        else if (strcmp(tokensOfUserInput[0], "rms") == 0)
        {
            vector_rms(elements, &SizeOfArray);
        }
        else if (strcmp(tokensOfUserInput[0], "neg") == 0)
        {
            vector_neg(elements, &SizeOfArray);
        }
        else if (strcmp(tokensOfUserInput[0], "add") == 0)
        {
            vector_add(elements, &SizeOfArray);
        }
        else if (strcmp(tokensOfUserInput[0], "sub") == 0)
        {
            vector_sub(elements, &SizeOfArray);
        }
